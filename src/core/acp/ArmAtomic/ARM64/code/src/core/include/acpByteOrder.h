...
ACP_INLINE acp_uint16_t acpByteSwap16(acp_uint16_t aVal)
{
	...
	acp_uint16_t sRes = ((aVal & 0x00FF) << 8) | ((aVal & 0xFF00) >> 8);
    __asm
    {
        mov ax, aVal;
        xchg ah, al;
...
ACP_INLINE acp_uint16_t acpByteSwap16(acp_uint16_t aVal)
{
	...
	sRes = ((aVal & 0x00FF) << 8) | ((aVal & 0xFF00) >> 8);
    return sRes;
}
...
ACP_INLINE acp_uint64_t acpByteSwap64(acp_uint64_t aVal)
{
   // __asm__("bswap %0" : "=r" (aVal) : "0" (aVal)); 
	acp_uint64_t sRes;
	sRes = ((((aVal) & 0xff00000000000000ull) >> 56)
      | (((aVal) & 0x00ff000000000000ull) >> 40)
      | (((aVal) & 0x0000ff0000000000ull) >> 24)
      | (((aVal) & 0x000000ff00000000ull) >> 8)
      | (((aVal) & 0x00000000ff000000ull) << 8)
      | (((aVal) & 0x0000000000ff0000ull) << 24)
      | (((aVal) & 0x000000000000ff00ull) << 40)				      \
      | (((aVal) & 0x00000000000000ffull) << 56));

    return sRes;
}
*/

// jhpark -- ARMv8 aarch64 inline assembly language
