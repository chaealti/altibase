...
// BUG-47487: FLI Page인지의 여부를 판단한다.
idBool svmExpandChunk::isFLIPageID( svmTBSNode * aTBSNode, 
                                    scPageID     aPageID )
{
    vULong sPageNoInChunk ;
    
    // Membase및 Catalog Table이 있는 Page
                                    ...
                                    idBool *     aIsFreePage )
{
    ...
    if ( isDataPageID( aTBSNode, aPageID ) )
    {
        ...
        // 테이블에 할당된 Page인 경우
        // Free List Info Page에 SVM_FLI_ALLOCATED_PID가 설정된다.
        // ( Page가 테이블로 할당될 때
        //   svmFPLManager::allocFreePageMemoryList 에서 설정됨 )
