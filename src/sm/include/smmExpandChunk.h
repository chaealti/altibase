...
/***********************************************************************
* $Id: smmExpandChunk.h 86424 2019-12-11 05:19:34Z jiwon.kim $
**********************************************************************/

#ifndef _O_SMM_EXPAND_CHUNK_H_
#define _O_SMM_EXPAND_CHUNK_H_ 1
...
typedef struct smmFLISlot
{
    // 이 Slot과 1:1 매칭되는 Page가
    // Database Free Page인 경우   ==> 다음 Free Page의 ID
    // Table Allocated Page인 경우 ==> SMM_FLI_ALLOCATED_PID
    //
...
#define SMM_FLI_SLOT_SIZE  ( ID_SIZEOF(smmFLISlot) )

// Expand Chunk
class smmExpandChunk
{
private:
    ...
    static scPageID getExpandChunkPID( smmTBSNode * aTBSNode,
    ...
    // Data Page No 를 리턴한다.
    static vULong getDataPageNoInChunk( smmTBSNode * aTBSNode,
                                        scPageID     aDataPageID );
...
public :
    ...
    static scPageID getFLIPageID( smmTBSNode * aTBSNode,
                                  scPageID     aDataPageID );
    
    // FLI Page 내에서 Data Page와 1:1로 매핑되는 Slot의 offset을 계산한다.
    static UInt getSlotOffsetInFLIPage( smmTBSNode * aTBSNode,
                                        scPageID     aDataPageID );
    
    // ExpandChunk당 Page수를 설정한다.
    static IDE_RC setChunkPageCnt( smmTBSNode * aTBSNode,
    ...
    static IDE_RC logAndSetNextFreePage( smmTBSNode * aTBSNode,
                                         void     *   aTrans,
                                         scPageID     aPageID,
                                         scPageID     aNextFreePageID );
    
    // Next Free Page ID를 가져온다.
    static IDE_RC getNextFreePage( smmTBSNode * aTBSNode,
                                   scPageID     aPageID,
                                   scPageID *   aNextFreePageID );

    // 최소한 특정 Page수 만큼 저장할 수 있는 Expand Chunk의 수를 계산한다.
    static vULong getExpandChunkCount( smmTBSNode * aTBSNode,
                                       vULong       aRequiredPageCnt );

    // Data Page인지의 여부를 판단한다.
    static idBool isDataPageID( smmTBSNode * aTBSNode,
                                scPageID     aPageID );

    // FLI Page인지의 여부를 판단한다.
    static idBool isFLIPageID( smmTBSNode * aTBSNode,
                                scPageID     aPageID );

    // 하나의 Chunk당 Free List Info Page의 수를 리턴한다.
    static UInt getChunkFLIPageCnt(smmTBSNode * aTBSNode);

    // 데이터베이스 Page가 Free Page인지 여부를 리턴한다.
    static IDE_RC isFreePageID(smmTBSNode * aTBSNode,
                               scPageID     aPageID,
                               idBool *     aIsFreePage );
    
    // BUG-31863 [sm-util] The dump utility is necessary to verify
    // FreePageList of memory tablespace.
    static UInt getFLISlotCount();

    /* BUG-32461 [sm-mem-resource] add getPageState functions to 
     * smmExpandChunk module */
    static IDE_RC getPageState( smmTBSNode     * aTBSNode,
                                scPageID         aPageID,
...
/* 하나의 Chunk당 Free List Info Page의 수를 리턴한다.
 */
inline UInt smmExpandChunk::getChunkFLIPageCnt(smmTBSNode * aTBSNode)
{
...
inline UInt smmExpandChunk::getFLISlotCount()
{
    return mFLISlotCount;
}

#endif /* _O_SMM_EXPAND_CHUNK_H_ */
