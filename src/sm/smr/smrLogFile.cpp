...
/* ------------------------------------------------
 * Description : LogFile을 준비 한다.(BUG-48409)
 *               logfile 생성 중에 서버가 비정상 종료 하면 
 *               생성 중이던 size가 맞지 않는 logfile 이 생긴다.
 *               prepare 중에 임시 파일명으로 생성해서
                           ...
                           UInt      aSize )
{
    ...
    /* BUG-48409 생성한 Logfile의 size를 확인한다. */    
                            ...
                            UInt     aOffsetToSync )
{
    ...
    if ( sInvalidSize > 0 )
    {
        ...
        {
            // sSyncSize가 한 페이지 안에 들어가는 작은 크기인 경우,
