...
//        sSMN = sdi::getSMNForDataNode();
        ...
        // Clone이나 Solo의 경우 default node name은 항상 null 이다.
        if ( idlOS::strlen( sDefaultNodeNameStr ) != 0 )
        {
            // get partition Type And default partition name
            // NULL String 입력시 Default PartitionName을 반환해줌
            ...
            /* default partition replication 은 default node가 지정된 partitioned table 이어야만 수행한다. */
