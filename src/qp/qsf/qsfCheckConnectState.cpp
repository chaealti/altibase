/** 
 *  Copyright (c) 1999~2017, Altibase Corp. and/or its affiliates. All rights reserved.
 *
 *  This program is free software: you can redistribute it and/or modify
 *  it under the terms of the GNU Affero General Public License, version 3,
 *  as published by the Free Software Foundation.
 *
 *  This program is distributed in the hope that it will be useful,
 *  but WITHOUT ANY WARRANTY; without even the implied warranty of
 *  MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE. See the
 *  GNU Affero General Public License for more details.
 *
 *  You should have received a copy of the GNU Affero General Public License
 *  along with this program. If not, see <http://www.gnu.org/licenses/>.
 */
 

/***********************************************************************
 * $Id: qsfCheckConnectState.cpp 84991 2019-03-11 09:21:00Z andrew.shin $
 **********************************************************************/

#include <idl.h>
#include <mte.h>
#include <mtc.h>
#include <mtd.h>
#include <qsf.h>
#include <mtk.h>
#include <mtv.h>
#include <mtdTypes.h>
#include <qtc.h>
#include <qsxUtil.h>
#include <qcuSessionObj.h>

extern mtdModule mtsConnect;
extern mtdModule mtdInteger;

static mtcName qsfFunctionName[1] = {
    { NULL, 19, (void*)"CHECK_CONNECT_STATE" }
};

static IDE_RC qsfEstimate( mtcNode     * aNode,
                           mtcTemplate * aTemplate,
                           mtcStack    * aStack,
                           SInt          aRemain,
                           mtcCallBack * aCallBack );

mtfModule qsfCheckConnectStateModule = {
    1 | MTC_NODE_OPERATOR_MISC | MTC_NODE_VARIABLE_TRUE,
    ~0,
    1.0, /* default selectivity (비교 연산자 아님) */
    qsfFunctionName,
    NULL,
    mtf::initializeDefault,
    mtf::finalizeDefault,
    qsfEstimate
};


IDE_RC qsfCalculate_CheckConnectState( mtcNode     * aNode,
                                       mtcStack    * aStack,
                                       SInt          aRemain,
                                       void        * aInfo,
                                       mtcTemplate * aTemplate );

static const mtcExecute qsfExecute = {
    mtf::calculateNA,
    mtf::calculateNA,
    mtf::calculateNA,
    mtf::calculateNA,
    qsfCalculate_CheckConnectState,
    NULL,
    mtx::calculateNA,
    mtk::estimateRangeNA,
    mtk::extractRangeNA
};


IDE_RC qsfEstimate( mtcNode     * aNode,
                    mtcTemplate * aTemplate,
                    mtcStack    * aStack,
                    SInt       /* aRemain */,
                    mtcCallBack * aCallBack )
{
    const mtdModule* sModules[2] =
    {
        &mtsConnect,
        &mtdInteger
    };

    const mtdModule* sModule = & mtdInteger;

    IDE_TEST_RAISE( ( aNode->lflag & MTC_NODE_QUANTIFIER_MASK ) == MTC_NODE_QUANTIFIER_TRUE,
                    ERR_NOT_AGGREGATION );

    IDE_TEST_RAISE( ( aNode->lflag & MTC_NODE_ARGUMENT_COUNT_MASK ) != 2,
                    ERR_INVALID_FUNCTION_ARGUMENT );

    IDE_TEST( mtf::makeConversionNodes( aNode,
                                        aNode->arguments,
                                        aTemplate,
                                        aStack + 1,
                                        aCallBack,
                                        sModules )
              != IDE_SUCCESS );

    aStack[0].column = aTemplate->rows[aNode->table].columns + aNode->column;

    /* Return값은 Integer */
    IDE_TEST( mtc::initializeColumn( aStack[0].column,
                                     sModule,
                                     0,
                                     0,
                                     0 )
              != IDE_SUCCESS );

    aTemplate->rows[aNode->table].execute[aNode->column] = qsfExecute;

    return IDE_SUCCESS;

    IDE_EXCEPTION( ERR_NOT_AGGREGATION );
    {
        IDE_SET( ideSetErrorCode( mtERR_ABORT_NOT_AGGREGATION ) );
    }
    IDE_EXCEPTION( ERR_INVALID_FUNCTION_ARGUMENT );
    {
        IDE_SET( ideSetErrorCode( mtERR_ABORT_INVALID_FUNCTION_ARGUMENT ) );
    }
    IDE_EXCEPTION_END;

    return IDE_FAILURE;
}

IDE_RC qsfCalculate_CheckConnectState( mtcNode     * aNode,
                                       mtcStack    * aStack,
                                       SInt          aRemain,
                                       void        * aInfo,
                                       mtcTemplate * aTemplate )
{
    qcStatement     * sStatement;
    qcSession       * sSession;
    mtdIntegerType  * sReturnValue;
    mtsConnectType  * sConnectType;
    SInt              sNextState;
    SInt              sCurrentState;

    sStatement = ((qcTemplate*)aTemplate)->stmt;

    sSession   = sStatement->spxEnv->mSession;

    IDU_FIT_POINT( "qsfCheckConnectState::qsfCalculate_CheckConnectState::coverage::1" );
    IDE_TEST( mtf::postfixCalculate( aNode,
                                     aStack,
                                     aRemain,
                                     aInfo,
                                     aTemplate )
              != IDE_SUCCESS );

    if ( ( aStack[1].column->module->isNull( aStack[1].column,
                                             aStack[1].value ) == ID_TRUE ) ||
         ( aStack[2].column->module->isNull( aStack[2].column,
                                             aStack[2].value ) == ID_TRUE ) )
    {
        /* error. value argument is invalid. */
        IDE_RAISE( ERR_ARGUMENT_NOT_APPLICABLE );
    }
    else
    {
        /* return value */
        sReturnValue  = (mtdIntegerType *)aStack[0].value;
        *sReturnValue = -1;

        /* connect type */
        sConnectType = (mtsConnectType *)aStack[1].value;

        /* next state value */
        sNextState = *(mtdIntegerType *)aStack[2].value;

        /* current state value */
        qcuSessionObj::getConnectionState( (qcSessionObjInfo*)(sSession->mQPSpecific.mSessionObj),
                                           sConnectType->connectionNodeKey,
                                           & sCurrentState );

        switch ( sCurrentState )
        {
            /* PROJ-2657 UTL_SMTP 지원
             * - SMTP 서버 상의 예외처리는 제외한다.
             */
            case QC_CONNECTION_STATE_NOCONNECT:
                IDE_RAISE( ERR_ARGUMENT_NOT_APPLICABLE );
                break;
            case QC_CONNECTION_STATE_CONNECTED:
                break;
            case QC_CONNECTION_STATE_SMTP_HELO:
            case QC_CONNECTION_STATE_SMTP_MAIL:
            case QC_CONNECTION_STATE_SMTP_RCPT:
            case QC_CONNECTION_STATE_SMTP_CRLF:
                switch ( sNextState )
                {
                    case QC_CONNECTION_STATE_CONNECTED:
                    case QC_CONNECTION_STATE_SMTP_HELO:
                    case QC_CONNECTION_STATE_SMTP_MAIL:
                    case QC_CONNECTION_STATE_SMTP_RCPT:
                    case QC_CONNECTION_STATE_SMTP_OPEN:
                        break;
                    default:
                        IDE_RAISE( ERR_INVALID_STATE );
                        break;
                }
                break;
            case QC_CONNECTION_STATE_SMTP_OPEN:
                switch ( sNextState )
                {
                    case QC_CONNECTION_STATE_SMTP_DATA:
                    case QC_CONNECTION_STATE_SMTP_CRLF:
                        break;
                    default:
                        IDE_RAISE( ERR_INVALID_STATE );
                        break;
                }
                break;
            case QC_CONNECTION_STATE_SMTP_DATA:
                switch ( sNextState )
                {
                    case QC_CONNECTION_STATE_SMTP_DATA:
                    case QC_CONNECTION_STATE_SMTP_CRLF:
                        break;
                    default:
                        IDE_RAISE( ERR_INVALID_STATE );
                        break;
                }
                break;
            default:
                IDE_RAISE( ERR_INVALID_STATE );
                break;
        }

        /* update current state value */
        qcuSessionObj::setConnectionState( (qcSessionObjInfo*)(sSession->mQPSpecific.mSessionObj),
                                           sConnectType->connectionNodeKey,
                                           sNextState );

        *sReturnValue = 0;
    }

    return IDE_SUCCESS;

    IDE_EXCEPTION( ERR_ARGUMENT_NOT_APPLICABLE );
    {
        IDE_SET( ideSetErrorCode( mtERR_ABORT_ARGUMENT_NOT_APPLICABLE ) );
    }
    IDE_EXCEPTION( ERR_INVALID_STATE );
    {
        IDE_SET( ideSetErrorCode( mtERR_ABORT_UNEXPECTED_ERROR,
                                  "qsfCalculate_CheckConnectState",
                                  "Unsupported connection state occurred" ) );
    }
    IDE_EXCEPTION_END;

    return IDE_FAILURE;
}

