...
IDE_RC qsfCalculate_SpUnKeepPlan( 
                            mtcNode*     aNode,
                            mtcStack*    aStack,
                            SInt         aRemain,
                            void*        aInfo,
                     ...
                     mtcTemplate* aTemplate )
{
    ...
    IDE_TEST( qci::mSessionCallback.mPlanCacheKeep( QC_MM_STMT(sStatement),
                                                    sBuffer,
                                                    ID_FALSE /* Unkeep */ ) != IDE_SUCCESS );

    *(mtdIntegerType *)aStack[0].value = 0;
 
    ideLog::log( IDE_QP_0, "unkeep_plan (\'%s\') success", sBuffer );
    ...
    ideLog::log( IDE_QP_0, "unkeep_plan (\'%s\') failure [ERR-%x : %s]", sBuffer,
