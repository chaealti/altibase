...
static mtcName stfMPointFromWKBFunctionName[2] = {
    { stfMPointFromWKBFunctionName+1, 16, (void*)"ST_MPOINTFROMWKB" },
    { NULL, 13, (void*)"MPOINTFROMWKB" }
};

static IDE_RC stfMPointFromWKBEstimate(
                        mtcNode*     aNode,
...
mtfModule stfMPointFromWKB = {
    ...
    mtf::finalizeDefault,
    stfMPointFromWKBEstimate
};

IDE_RC stfMPointFromWKBCalculate(
                        mtcNode*     aNode,
                        mtcStack*    aStack,
                        SInt         aRemain,
                        void*        aInfo,
...
IDE_RC stfMPointFromWKBEstimate(
                        mtcNode*     aNode,
                        mtcTemplate* aTemplate,
                        mtcStack*    aStack,
                        SInt      /* aRemain */,
                        mtcCallBack* aCallBack )
{
    ...
    *sModules = &mtdBinary;
    ...
    {
        // max point count = (total wkb size - wkb_heder size)/wkb pointtype size.
        sMaxHeaderCount = (aStack[1].column->precision -
                           WKB_GEOHEAD_SIZE- WKB_INT32_SIZE) /
            (WKB_POINT_SIZE);
        sNativeGeomSize = (sMaxHeaderCount * STD_POINT2D_SIZE )
            + STD_MLINE2D_SIZE;
    }
