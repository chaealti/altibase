...
static mtcName stfNumInteriorRingFunctionName[2] = {
    { stfNumInteriorRingFunctionName+1, 18, (void*)"ST_NUMINTERIORRING" }, // Fix BUG-15519
    { NULL, 15, (void*)"NUMINTERIORRING" }
};

static IDE_RC stfNumInteriorRingEstimate(
                        mtcNode*     aNode,
...
mtfModule stfNumInteriorRing = {
    1|MTC_NODE_OPERATOR_FUNCTION,
    ~(MTC_NODE_INDEX_MASK),
    1.0,  // default selectivity (비교 연산자가 아님)
    stfNumInteriorRingFunctionName,
    NULL,
    mtf::initializeDefault,
    mtf::finalizeDefault,
    stfNumInteriorRingEstimate
};

IDE_RC stfNumInteriorRingCalculate(
                        mtcNode*     aNode,
                        mtcStack*    aStack,
                        SInt         aRemain,
                        void*        aInfo,
...
IDE_RC stfNumInteriorRingEstimate(
                        mtcNode*     aNode,
                        mtcTemplate* aTemplate,
                        mtcStack*    aStack,
                        SInt      /* aRemain */,
                        ...
                        mtcTemplate* aTemplate )
{
    ...
    IDE_TEST( stfFunctions::numInteriorRing( aNode,
                                             aStack,
                                             aRemain,
                                             aInfo,
