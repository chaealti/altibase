...
static mtcName stfPointOnSurfaceFunctionName[2] = {
    { stfPointOnSurfaceFunctionName+1, 17, (void*)"ST_POINTONSURFACE" }, // Fix BUG-15519
    { NULL, 14, (void*)"POINTONSURFACE" }
};

static IDE_RC stfPointOnSurfaceEstimate(
                        mtcNode*     aNode,
...
mtfModule stfPointOnSurface = {
    1|MTC_NODE_OPERATOR_FUNCTION,
    ~(MTC_NODE_INDEX_MASK),
    1.0,  // default selectivity (비교 연산자가 아님)
    stfPointOnSurfaceFunctionName,
    NULL,
    mtf::initializeDefault,
    mtf::finalizeDefault,
    stfPointOnSurfaceEstimate
};

IDE_RC stfPointOnSurfaceCalculate(
                        mtcNode*     aNode,
                        mtcStack*    aStack,
                        SInt         aRemain,
                        void*        aInfo,
...
IDE_RC stfPointOnSurfaceEstimate(
                        mtcNode*     aNode,
                        mtcTemplate* aTemplate,
                        mtcStack*    aStack,
                        SInt      /* aRemain */,
                        ...
                        mtcTemplate* aTemplate )
{
    ...
    IDE_TEST( stfFunctions::pointOnSurface( aNode,
                                            aStack,
                                            aRemain,
                                            aInfo,
