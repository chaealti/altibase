...
static mtcName stfPointFromTextFunctionName[2] = {
    { stfPointFromTextFunctionName+1, 16, (void*)"ST_POINTFROMTEXT" }, // Fix BUG-15519
    { NULL, 13, (void*)"POINTFROMTEXT" }
};

static IDE_RC stfPointFromTextEstimate(
                        mtcNode*     aNode,
...
mtfModule stfPointFromText = {
    1|MTC_NODE_OPERATOR_FUNCTION,
    ~(MTC_NODE_INDEX_MASK),
    1.0,  // default selectivity (비교 연산자가 아님)
    stfPointFromTextFunctionName,
    NULL,
    mtf::initializeDefault,
    mtf::finalizeDefault,
    stfPointFromTextEstimate
};

IDE_RC stfPointFromTextCalculate(
                        mtcNode*     aNode,
                        mtcStack*    aStack,
                        SInt         aRemain,
                        void*        aInfo,
...
IDE_RC stfPointFromTextEstimate(
                        mtcNode*     aNode,
                        mtcTemplate* aTemplate,
                        mtcStack*    aStack,
                        SInt      /* aRemain */,
                        ...
                        mtcTemplate* aTemplate )
{
    ...
    else
    {
        ...
        IDE_TEST( stfWKT::pointFromText( sQmxMem,
                                         aStack[1].value,
                                         aStack[0].value,
