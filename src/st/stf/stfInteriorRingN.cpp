...
static mtcName stfInteriorRingNFunctionName[2] = {
    { stfInteriorRingNFunctionName+1, 16, (void*)"ST_INTERIORRINGN" }, // Fix BUG-15519
    { NULL, 13, (void*)"INTERIORRINGN" }
};

static IDE_RC stfInteriorRingNEstimate(
                        mtcNode*     aNode,
...
mtfModule stfInteriorRingN = {
    1|MTC_NODE_OPERATOR_FUNCTION,
    ~(MTC_NODE_INDEX_MASK),
    1.0,  // default selectivity (비교 연산자가 아님)
    stfInteriorRingNFunctionName,
    NULL,
    mtf::initializeDefault,
    mtf::finalizeDefault,
    stfInteriorRingNEstimate
};

IDE_RC stfInteriorRingNCalculate(
                        mtcNode*     aNode,
                        mtcStack*    aStack,
                        SInt         aRemain,
                        void*        aInfo,
...
IDE_RC stfInteriorRingNEstimate(
                        mtcNode*     aNode,
                        mtcTemplate* aTemplate,
                        mtcStack*    aStack,
                        SInt      /* aRemain */,
                        ...
                        mtcTemplate* aTemplate )
{
    ...
    IDE_TEST( stfFunctions::interiorRingN( aNode,
                                           aStack,
                                           aRemain,
                                           aInfo,
