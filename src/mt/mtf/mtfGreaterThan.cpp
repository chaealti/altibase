...
/***********************************************************************
 * $Id: mtfGreaterThan.cpp 86698 2020-02-18 07:17:43Z donovan.seo $
 **********************************************************************/
...
static mtcName mtfGreaterThanFunctionName[1] = {
    { NULL, 1, (void*)">" }
};

static IDE_RC mtfGreaterThanEstimate( mtcNode*     aNode,
                                      mtcTemplate* aTemplate,
                                      mtcStack*    aStack,
                                      SInt         aRemain,
                                      mtcCallBack* aCallBack );

mtfModule mtfGreaterThan = {
    1|MTC_NODE_OPERATOR_GREATER|MTC_NODE_COMPARISON_TRUE|
        MTC_NODE_INDEX_ARGUMENT_BOTH|MTC_NODE_INDEX_JOINABLE_TRUE|
        MTC_NODE_PRINT_FMT_INFIX,
    ~0,
    1.0/3.0,  // TODO : default selectivity 
    mtfGreaterThanFunctionName,
    &mtfLessEqual,
    mtf::initializeDefault,
    mtf::finalizeDefault,
    mtfGreaterThanEstimate
};

IDE_RC mtfGreaterThanExtractRange( mtcNode*       aNode,
                                   mtcTemplate*   aTemplate,
                                   mtkRangeInfo * aInfo,
                                   smiRange*      aRange );

IDE_RC mtfGreaterThanCalculate( mtcNode*     aNode,
                                mtcStack*    aStack,
                                SInt         aRemain,
                                void*        aInfo,
                                mtcTemplate* aTemplate );
...
IDE_RC mtfGreaterThanCalculateList( mtcNode     * aNode,
                                    mtcStack    * aStack,
                                    SInt          aRemain,
                                    void        * aInfo,
                                    mtcTemplate * aTemplate );
...
static const mtcExecute mtfExecute = {
    ...
    mtf::calculateNA,
    mtf::calculateNA,
    mtfGreaterThanCalculate,
    NULL,
    mtx::calculateNA,
    mtk::estimateRangeDefault,
    mtfGreaterThanExtractRange
};
...
IDE_RC mtfGreaterThanEstimate( mtcNode*     aNode,
                               mtcTemplate* aTemplate,
                               mtcStack*    aStack,
                               SInt      /* aRemain */,
                               mtcCallBack* aCallBack )
         ...
         == MTC_ESTIMATE_ARGUMENTS_ENABLE )
    {
        ...
        if ( aStack[1].column->module != &mtdList )
        {
            ...
            aTemplate->rows[aNode->table].execute[aNode->column].mSerialExecute
                = mtxGreaterThan.mGetExecute( sModules[0]->id, sModules[1]->id );
        }
        else
        {
            /* BUG-39153 support to comparison operators for list type */
