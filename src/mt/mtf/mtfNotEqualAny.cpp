...
/***********************************************************************
 * $Id: mtfNotEqualAny.cpp 84991 2019-03-11 09:21:00Z andrew.shin $
 **********************************************************************/
...
static mtcName mtfNotEqualAnyFunctionName[1] = {
    { NULL, 5, (void*)"<>ANY" }
};

static IDE_RC mtfNotEqualAnyEstimate( mtcNode*     aNode,
...
mtfModule mtfNotEqualAny = {
    2|MTC_NODE_OPERATOR_NOT_EQUAL|
    ...
    mtfNotEqualAnyFunctionName,
    &mtfEqualAll,
    mtf::initializeDefault,
    mtf::finalizeDefault,
    mtfNotEqualAnyEstimate
};

IDE_RC mtfNotEqualAnyCalculateSelect( mtcNode *     aNode,
                                      mtcStack *    aStack,
                                      SInt          aRemain,
                                      void *        aInfo,
                                      mtcTemplate * aTemplate );

IDE_RC mtfNotEqualAnyCalculateFast( mtcNode *     aNode,
                                    mtcStack *    aStack,
                                    SInt          aRemain,
                                    void *        aInfo,
                                    mtcTemplate * aTemplate );

IDE_RC mtfNotEqualAnyCalculate( mtcNode*     aNode,
                                mtcStack*    aStack,
                                SInt         aRemain,
                                void*        aInfo,
                                mtcTemplate* aTemplate );

IDE_RC mtfNotEqualAnyCalculateList( mtcNode*     aNode,
                                    mtcStack*    aStack,
                                    SInt         aRemain,
                                    void*        aInfo,
                                    mtcTemplate* aTemplate );

IDE_RC mtfNotEqualAnyCalculateSubquery( mtcNode*     aNode,
                                        mtcStack*    aStack,
                                        SInt         aRemain,
                                        void*        aInfo,
                                        mtcTemplate* aTemplate );

IDE_RC mtfNotEqualAnyCalculateSubqueryList( mtcNode*     aNode,
                                            mtcStack*    aStack,
                                            SInt         aRemain,
                                            void*        aInfo,
                                            mtcTemplate* aTemplate );
...
static const mtcExecute mtfExecuteSubquery = {
    ...
    mtf::calculateNA,
    mtf::calculateNA,
    mtfNotEqualAnyCalculateSubquery,
    NULL,
    mtx::calculateNA,
    mtk::estimateRangeNA,
    mtk::extractRangeNA
...
static const mtcExecute mtfExecuteSubqueryList = {
    ...
    mtf::calculateNA,
    mtf::calculateNA,
    mtfNotEqualAnyCalculateSubqueryList,
    NULL,
    mtx::calculateNA,
    mtk::estimateRangeNA,
    mtk::extractRangeNA
...
IDE_RC mtfNotEqualAnyEstimate( mtcNode*     aNode,
                               mtcTemplate* aTemplate,
                               mtcStack*    aStack,
                               SInt      /* aRemain */,
                               mtcCallBack* aCallBack )
                                      ...
                                      mtcTemplate * aTemplate )
{
    ...
    sFastCalcFunc   = mtfNotEqualAnyCalculateFast;
    sNormalCalcFunc = mtfNotEqualAnyCalculate;
                                    ...
                                    mtcTemplate * aTemplate )
{
    ...
    /* Right Values are Sorted Value,
     * Thus, needs check only ( LValue == First RValue == Last RValue ) */
    ...
    else
    {
        ...
        if ( sValue != MTD_BOOLEAN_NULL )
        {
            ...
            sValueInfo3.column = sStackList[sLast].column;
            sValueInfo3.value  = sStackList[sLast].value;
            sValueInfo3.flag   = MTD_OFFSET_USELESS;

            if ( sMtdModule->logicalCompare[MTD_COMPARE_ASCENDING](
                                                     &sValueInfo1,
                                                     &sValueInfo2 ) == 0 &&
                 sMtdModule->logicalCompare[MTD_COMPARE_ASCENDING](
                                                     &sValueInfo1,
                                                     &sValueInfo3 ) == 0 )
            {
                                    ...
                                    mtcTemplate* aTemplate )
{
    ...
    {
        sSubValue = MTD_BOOLEAN_FALSE;
        for( sCount1 = 0,
             sNode3  = sNode2->arguments,
             sStack3 = (mtcStack*)sStack2[sCount2].value;
             sCount1 < sFence1 && sSubValue != MTD_BOOLEAN_TRUE;
             sCount1++, sNode3 = sNode3->next )
        {
            ...
            {
                sSubValue = mtf::orMatrix[sSubValue][MTD_BOOLEAN_NULL];
            }
            else
            {
                sValueInfo1.column = sStack.column;
                ...
                if ( sModule->logicalCompare[MTD_COMPARE_ASCENDING]( &sValueInfo1,
                                                                     &sValueInfo2 ) != 0 )
                {
                    sSubValue = mtf::orMatrix[sSubValue][MTD_BOOLEAN_TRUE];
                }
                else
                {
                    sSubValue = mtf::orMatrix[sSubValue][MTD_BOOLEAN_FALSE];
                }
            }
        }
                                            ...
                                            mtcTemplate* aTemplate )
{
    ...
    while( sRowExist == ID_TRUE && sValue != MTD_BOOLEAN_TRUE )
    {
        sSubValue = MTD_BOOLEAN_FALSE;
        for( sCount = 0;
             sCount < sFence && sSubValue != MTD_BOOLEAN_TRUE;
             sCount++ )
        {
