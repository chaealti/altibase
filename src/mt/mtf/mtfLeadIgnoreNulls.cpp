...
static mtcName mtfLeadIgnoreNullsName[1] = {
    { NULL,  17, ( void * )"LEAD_IGNORE_NULLS" }
};

static IDE_RC mtfLeadIgnoreNullsEstimate( mtcNode     * aNode,
                                          mtcTemplate * aTemplate,
                                          mtcStack    * aStack,
                                          SInt          aRemain,
                                          mtcCallBack * aCallBack );

mtfModule mtfLeadIgnoreNulls = {
    2 | MTC_NODE_OPERATOR_AGGREGATION  |
    MTC_NODE_FUNCTION_ANALYTIC_TRUE |
    MTC_NODE_FUNCTION_RANKING_TRUE,
    ~( MTC_NODE_INDEX_MASK ),
    1.0, /* default selectivity ( 비교 연산자가 아님 ) */
    mtfLeadIgnoreNullsName,
    NULL,
    mtf::initializeDefault,
    mtf::finalizeDefault,
    mtfLeadIgnoreNullsEstimate
};

IDE_RC mtfLeadIgnoreNullsInitialize( mtcNode     * aNode,
                                     mtcStack    * aStack,
                                     SInt          aRemain,
                                     void        * aInfo,
                                     mtcTemplate * aTemplate );

IDE_RC mtfLeadIgnoreNullsAggregate( mtcNode     * aNode,
                                    mtcStack    * aStack,
                                    SInt          aRemain,
                                    void        * aInfo,
                                    mtcTemplate * aTemplate );

IDE_RC mtfLeadIgnoreNullsFinalize( mtcNode     * aNode,
                                   mtcStack    * aStack,
                                   SInt          aRemain,
                                   void        * aInfo,
                                   mtcTemplate * aTemplate );

IDE_RC mtfLeadIgnoreNullsCalculate( mtcNode     * aNode,
                                    mtcStack    * aStack,
                                    SInt          aRemain,
                                    void        * aInfo,
                                    mtcTemplate * aTemplate );
...
static const mtcExecute mtfExecute = {
    mtfLeadIgnoreNullsInitialize,
    mtfLeadIgnoreNullsAggregate,
    mtf::calculateNA,
...
IDE_RC mtfLeadIgnoreNullsEstimate( mtcNode     * aNode,
                                   mtcTemplate * aTemplate,
                                   mtcStack    * aStack,
                                   SInt          /* aRemain */,
                                   mtcCallBack * aCallBack )
