...
static mtcName mtfCumeDistWGFunctionName[ 1 ] = {
    { NULL, 9, (void*)"CUME_DIST" }
};

static IDE_RC mtfCumeDistWGEstimate( mtcNode     *  aNode,
...
mtfModule mtfCumeDistWithinGroup = {
    ...
    mtf::finalizeDefault,
    mtfCumeDistWGEstimate
};

IDE_RC mtfCumeDistWGInitialize( mtcNode     * aNode,
                                mtcStack    * aStack,
                                SInt          aRemain,
                                void        * aInfo,
                                mtcTemplate * aTemplate );

IDE_RC mtfCumeDistWGAggregate( mtcNode     * aNode,
                               mtcStack    * aStack,
                               SInt          aRemain,
                               void        * aInfo,
                               mtcTemplate * aTemplate );

IDE_RC mtfCumeDistWGMerge( mtcNode     * aNode,
                           mtcStack    * aStack,
                           SInt          aRemain,
                           void        * aInfo,
                           mtcTemplate * aTemplate );

IDE_RC mtfCumeDistWGFinalize( mtcNode      * aNode,
                              mtcStack     * aStack,
                              SInt           aRemain,
                              void         * aInfo,
                              mtcTemplate  * aTemplate );

IDE_RC mtfCumeDistWGCalculate( mtcNode     * aNode,
                               mtcStack    * aStack,
                               SInt          aRemain,
                               void        * aInfo,
                               mtcTemplate * aTemplate );
...
static const mtcExecute mtfExecute = {
    mtfCumeDistWGInitialize,
    mtfCumeDistWGAggregate,
    mtfCumeDistWGMerge,
    mtfCumeDistWGFinalize,
    mtfCumeDistWGCalculate,
    NULL,
    mtx::calculateNA,
    mtk::estimateRangeNA,
    mtk::extractRangeNA
...
IDE_RC mtfCumeDistWGEstimate( mtcNode     * aNode,
                              mtcTemplate * aTemplate,
                              mtcStack    * aStack,
                              SInt,
                              mtcCallBack * aCallBack )
{
    ...
    // cume_dist() 의 인자 수 == within group() 의 인자 수.
    ...
    {
        // cume_dist(..) 와 Within Group(..) 의 각각 대응되는 인자의 대표타입을 구하여 설정한다.
                           ...
                           mtcTemplate * aTemplate )
{
    ...
    mtdBigintType   * sDstSame = NULL;
    mtdBigintType   * sSrcSame = NULL;
    ...
    *sDstRank += ( *sSrcRank - 1 );
    ...
    IDE_TEST_RAISE( ( MTD_BIGINT_MAXIMUM - *sDstSame ) < ( *sSrcSame ),
                    ERR_VALUE_OVERFLOW );
                              ...
                              mtcTemplate * aTemplate )
{
    ...
    sTotalRows = *( mtdBigintType * )( sRow + sColumn[ 3 ].column.offset );
    if ( sTotalRows > 1 )
    {
