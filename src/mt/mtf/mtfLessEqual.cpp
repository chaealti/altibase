...
/***********************************************************************
 * $Id: mtfLessEqual.cpp 86698 2020-02-18 07:17:43Z donovan.seo $
 **********************************************************************/
...
static mtcName mtfLessEqualFunctionName[1] = {
    { NULL, 2, (void*)"<=" }
};

static IDE_RC mtfLessEqualEstimate( mtcNode*     aNode,
                                    mtcTemplate* aTemplate,
                                    mtcStack*    aStack,
                                    SInt         aRemain,
                                    mtcCallBack* aCallBack );

mtfModule mtfLessEqual = {
    1|MTC_NODE_OPERATOR_LESS_EQUAL|MTC_NODE_COMPARISON_TRUE|
        MTC_NODE_INDEX_ARGUMENT_BOTH|MTC_NODE_INDEX_JOINABLE_TRUE|
        MTC_NODE_PRINT_FMT_INFIX,
    ~0,
    1.0/3.0,  // TODO : default selectivity 
    mtfLessEqualFunctionName,
    &mtfGreaterThan,
    mtf::initializeDefault,
    mtf::finalizeDefault,
    mtfLessEqualEstimate
};

IDE_RC mtfLessEqualExtractRange( mtcNode*       aNode,
                                 mtcTemplate*   aTemplate,
                                 mtkRangeInfo * aInfo,
                                 smiRange*      aRange );

IDE_RC mtfLessEqualCalculate( mtcNode*     aNode,
                              mtcStack*    aStack,
                              SInt         aRemain,
                              void*        aInfo,
                              mtcTemplate* aTemplate );
...
IDE_RC mtfLessEqualCalculateList( mtcNode     * aNode,
                                  mtcStack    * aStack,
                                  SInt          aRemain,
                                  void        * aInfo,
                                  mtcTemplate * aTemplate );
...
static const mtcExecute mtfExecute = {
    ...
    mtf::calculateNA,
    mtf::calculateNA,
    mtfLessEqualCalculate,
    NULL,
    mtx::calculateNA,
    mtk::estimateRangeDefault,
    mtfLessEqualExtractRange
};
...
IDE_RC mtfLessEqualEstimate( mtcNode*     aNode,
                             mtcTemplate* aTemplate,
                             mtcStack*    aStack,
                             SInt      /* aRemain */,
                             mtcCallBack* aCallBack )
         ...
         == MTC_ESTIMATE_ARGUMENTS_ENABLE )
    {
        ...
        if ( aStack[1].column->module != &mtdList )
        {
            ...
            aTemplate->rows[aNode->table].execute[aNode->column].mSerialExecute
                = mtxLessEqual.mGetExecute( sModules[0]->id, sModules[1]->id );
        }
        else
        {
            /* BUG-39153 support to comparison operators for list type */
