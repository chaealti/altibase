...
#define MTF_NTH_VALUE_INITIALIZE (-1)
#define MTF_NTH_VALUE_FIANLIZE   (-2)

static mtcName mtfNthValueName[1] = {
    { NULL,  9, ( void * )"NTH_VALUE" }
};

static IDE_RC mtfNthValueEstimate( mtcNode     * aNode,
                                   mtcTemplate * aTemplate,
                                   mtcStack    * aStack,
                                   SInt          aRemain,
                                   mtcCallBack * aCallBack );

mtfModule mtfNthValue = {
...
IDE_RC mtfNthValueInitialize( mtcNode     * aNode,
                              mtcStack    * aStack,
                              SInt          aRemain,
                              void        * aInfo,
                              mtcTemplate * aTemplate );

IDE_RC mtfNthValueAggregate( mtcNode     * aNode,
                             mtcStack    * aStack,
                             SInt          aRemain,
                             void        * aInfo,
                             mtcTemplate * aTemplate );

IDE_RC mtfNthValueFinalize( mtcNode     * aNode,
                            mtcStack    * aStack,
                            SInt          aRemain,
                            void        * aInfo,
                            mtcTemplate * aTemplate );

IDE_RC mtfNthValueCalculate( mtcNode     * aNode,
                             mtcStack    * aStack,
                             SInt          aRemain,
                             void        * aInfo,
                             mtcTemplate * aTemplate );
...
static const mtcExecute mtfExecute = {
    mtfNthValueInitialize,
    mtfNthValueAggregate,
    mtf::calculateNA,
    mtfNthValueFinalize,
    mtfNthValueCalculate,
    NULL,
    mtx::calculateNA,
    mtk::estimateRangeNA,
    mtk::extractRangeNA
...
IDE_RC mtfNthValueEstimate( mtcNode     * aNode,
                            mtcTemplate * aTemplate,
                            mtcStack    * aStack,
                            SInt          /* aRemain */,
                            mtcCallBack * aCallBack )
                              ...
                              mtcTemplate * aTemplate )
{
    ...
    *(mtdBigintType*) ((UChar*)aTemplate->rows[aNode->table].row
                        + sColumn[1].column.offset) = MTF_NTH_VALUE_INITIALIZE;
                             ...
                             mtcTemplate * aTemplate )
{
    ...
    if ( sCount == MTF_NTH_VALUE_INITIALIZE )
    {
    ...
    if ( sCount == 1 )
    {
        ...
        idlOS::memcpy( aStack[0].value,
                       ...
                       sModule->actualSize( aStack[1].column,
                                            aStack[1].value ) );
        *(mtdBigintType*) ((UChar*)aTemplate->rows[aNode->table].row
                            + sColumn[1].column.offset) = MTF_NTH_VALUE_FIANLIZE;
    }
    else
    {
        /* Nothing to do */
