...
void rpcDDLSyncManager::restoreTableInfo( smiTrans * aTrans, rpcDDLTableInfo * aDDLTableInfo )
{
    ...
    if ( aDDLTableInfo->mOldTableInfo != NULL )
    {
        ...
        qciMisc::restoreTempInfo( aDDLTableInfo->mOldTableInfo,
                                  /* BUGBUG partitionList 를 받지만 위에서 partition 을 직접 정리하므로 NULL 로 한다. */
                                  NULL,  
                                  NULL );
...
/*
 * 1. qciMisc::getPartitionInfoList() 호출하여 aTableID의 파티션드 테이블 리스트를 얻어온다. (DDL후에 현재 정보임)
 * 2. 상위에서 미리 저장해놨던 기존 파티션 리스트인 aDDLSyncPartInfoList와 루프를 돌면서 비교한다. (DDL전 백업 본)
 * 3. 두 리스트에서 tableOID가 같은 경우 mOldTableInfo가 NULL이 아니면 DDL 수행한 테이블 임으로 destory한다.
 * 4. aDDLSyncPartInfoList 순회가 끝났는데 못찾았으면 새로 생성된 파티션 테이블이므로 destory 한다.
 * */
