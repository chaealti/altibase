    ...
    do
        ...
        ${ISQL} -s ${sNode_ip} -port ${sNode_port} << EOF
            ...
            SET FEED OFF;
                ...
                exec utl_shard_online_rebuild.CHECK_EXIST_PARTITION( '${USER_NAME_ARRAY[$i]}',
                                                                     '${TABLE_NAME_ARRAY[$i]}',
                                                                     '${PARTITION_NAME_ARRAY[$i]}',
                                                                     TRUE );
                exec utl_shard_online_rebuild.CHECK_PARTITION_AND_SHARD_METHOD( '${USER_NAME_ARRAY[$i]}',
                                                                                '${TABLE_NAME_ARRAY[$i]}' );
                exec utl_shard_online_rebuild.CHECK_PARTITION_AND_SHARD_KEY_COLUMN( '${USER_NAME_ARRAY[$i]}',
                                                                                    '${TABLE_NAME_ARRAY[$i]}' );
                exec utl_shard_online_rebuild.CHECK_PARTITION_AND_SHARD_KEY_VALUE( '${USER_NAME_ARRAY[$i]}',
                                                                                   '${TABLE_NAME_ARRAY[$i]}' );
EOF
)
...
function check_table_row_count()
{
    local sNode_ip=${1}
    local sNode_port=${2}
    ...
    do
        ...
        ${ISQL} -s ${sNode_ip} -port ${sNode_port} << EOF
            ...
            SELECT * 
            FROM node[meta]( SELECT COUNT(*) 
                             FROM ${USER_NAME_ARRAY[$i]}.${TABLE_NAME_ARRAY[$i]}
                             PARTITION ( ${PARTITION_NAME_ARRAY[$i]} ) );
EOF
)

        if [ "${check}" -ne 0 ]; then
            echo ${USER_NAME_ARRAY[$i]}.${TABLE_NAME_ARRAY[$i]}.${PARTITION_NAME_ARRAY[$i]}' row count :'${check}' -> Failure'
            sFailiure=1
        else
            echo ${USER_NAME_ARRAY[$i]}.${TABLE_NAME_ARRAY[$i]}.${PARTITION_NAME_ARRAY[$i]}' row count :'${check}' -> Success'
            sFailiure=0
        fi
    done
...
EOF
        ...
        check_table_row_count ${NODE_IP_ARRAY[${i}]} ${NODE_PORT_ARRAY[${i}]}
        result=$?
        if [ "${result}" -ne 0 ]; then
            failiure=1
        fi
...
EOF
            ...
            check_table_row_count ${NODE_ALTERNATE_IP_ARRAY[${i}]} ${NODE_ALTERNATE_PORT_ARRAY[${i}]}
            result=$?
            if [ "${result}" -ne 0 ]; then
                failiure=1
            fi
