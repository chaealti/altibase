...
function check_table()
{
    ...
    do
        ...
        ${ISQL} -s ${sNode_ip} -port ${sNode_port} << EOF
            ...
            exec utl_shard_online_rebuild.CHECK_EXIST_CLONE( '${USER_NAME_ARRAY[$i]}',
                                                             '${TABLE_NAME_ARRAY[$i]}',
                                                             TRUE );

			exec utl_shard_online_rebuild.CHECK_CLONE( '${USER_NAME_ARRAY[$i]}',
													   '${TABLE_NAME_ARRAY[$i]}',
                                                       '${FROM_NODE_NAME}',
													   TRUE );
...
EOF
        ...
        check_table ${NODE_IP_ARRAY[$i]} ${NODE_PORT_ARRAY[$i]} 'TRUE'
        result=$?
...
EOF
            ...
            check_table ${NODE_IP_ARRAY[$i]} ${NODE_PORT_ARRAY[$i]} 'FALSE'
            result=$?
            if [ "${result}" -ne 0 ]; then
                failiure=1
            fi
