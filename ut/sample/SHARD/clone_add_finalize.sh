...
DIR_PATH=$(cd `dirname $0` && pwd)
/usr/bin/env bash ${DIR_PATH}/clone_add_check_finalize.sh ${1}
if [ $? = 1 ]; then
...
#   (Node2 -> Node3) Shard Data Replication을 정지하고 제거한다. (T2 -> T2_REBUILD)
echo ""
...
# Node3(NodeC)에서 T2_REBUILD를 제거한다.
#   (Node3 -> NodeC) Shard Data Replication을 정지하고 제거한다. (T2_REBUILD -> T2_REBUILD)
#   Node3(NodeC)에서 T2_REBUILD를 제거한다.
echo ""
