...
DIR_PATH=$(cd `dirname $0` && pwd)
/usr/bin/env bash ${DIR_PATH}/clone_add_check_main.sh ${1}
if [ $? = 1 ]; then
...
LOCK_AND_RELEASE_TABLE()
{
    ...
    ${ISQL_FROM_NODE} << EOF
        ...
        EXEC utl_shard_online_rebuild.lock_and_release( '${USER_NAME}',
                                                        '${TABLE_NAME}',
                                                        NULL,
                                                        '${REP_NAME_REBUILD}',
...
LOCK_AND_SWAP_TABLE()
{
    # Node3에서 T2에 X Lock을 잡는다. (Autocommit Off)
    # Rebuild Data Step이 성공/실패로 변경될 때까지 대기한다.
    # Node3에서 T2와 T2_REBUILD를 Swap한다. (Commit)
    # Replication이 Standby Node에서 T2와 T2_REBUILD를 Swap한다.
...
EOF
    ...
    # 성공한 경우 LOCK_AND_RELEASE_TABLE()에서 대기 중인 작업을 진행한다.
    NODE_SQL_RESULT=$(
        ${ISQL_TO_NODE} << EOF
            SET HEADING OFF;
...
do
    ...
    LOCK_AND_RELEASE_TABLE ${REBUILD_DATA_STEP_LOCKED} ${USER_NAME_ARRAY[$i]} ${TABLE_NAME_ARRAY[$i]} &
...
echo "[Step-7. (Target nodes) Release lock]"
echo "Node Info. : ${FROM_NODE_NAME}, ${FROM_NODE_IP}, ${FROM_NODE_PORT}"
