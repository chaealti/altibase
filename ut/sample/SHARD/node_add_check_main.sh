...
then
    echo $#
    echo "Usage : ${APPLICATION} shard_meta_server shard_meta_port sys_password new_node_name"
    exit 1
fi
...
do
    ...
    check=$( 
        ${ISQL} -s ${NODE_IP} -port ${NODE_PORT} << EOF
            SET HEADING OFF;
            SET FEED OFF;
            exec utl_shard_online_rebuild.CHECK_GLOBAL_META_INFO();
            exec utl_shard_online_rebuild.CHECK_EXIST_NODE( '${NEW_NODE_NAME}', TRUE );
            quit
EOF
)
