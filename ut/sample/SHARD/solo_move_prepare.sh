...
DIR_PATH=$(cd `dirname $0` && pwd)
/usr/bin/env bash ${DIR_PATH}/solo_move_check_prepare.sh ${1}
if [ $? = 1 ]; then
...
# 모든 Node에서 T3의 Node를 Node3(NodeC)로 변경한다.
echo ""
...
# Node3(NodeC)에 T3의 Schema를 복제한 T3_REBUILD를 생성한다.
echo ""
...
if [ ${TO_NODE_ALTERNATE_IP} ]
then
    # (Node3 -> NodeC) Shard Data Replication을 생성하고 시작한다. (T3_REBUILD -> T3_REBUILD)
...
# Node2의 T3를 Node3의 T3_REBUILD로 복제하기 시작한다. (Replication Sync)
#   (Node2 -> Node3) Shard Data Replication을 생성하고 시작한다. (T3 -> T3_REBUILD)
