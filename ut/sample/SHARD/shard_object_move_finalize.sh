...
DIR_PATH=$(cd `dirname $0` && pwd)
/usr/bin/env bash ${DIR_PATH}/shard_object_move_check_finalize.sh ${1}
if [ $? = 1 ]; then
...
#   (Node2 -> Node3) Shard Data Replication을 정지하고 제거한다. (T1(P3) -> T1_REBUILD(P3))
echo ""
...
# Node3(NodeC)에서 T1_REBUILD를 제거한다.
#   (Node3 -> NodeC) Shard Data Replication을 정지하고 제거한다. (T1_REBUILD(P3) -> T1_REBUILD(P3))
#   Node3(NodeC)에서 T1_REBUILD를 제거한다.
echo ""
