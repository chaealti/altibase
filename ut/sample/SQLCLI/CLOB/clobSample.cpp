...
SQLRETURN execute_clob_insert(SQLHDBC dbc, int src_file_id, const char *src_filename);
SQLRETURN execute_clob_select(SQLHDBC dbc, int desc_file_id, const char *desc_filename);
...
int main(int argc, char *argv[])
{
    ...
    rc = execute_clob_insert(dbc, file_id, argv[2]);
    if (!SQL_SUCCEEDED(rc))
    {
        goto EXIT_DBC;
    }

    rc = execute_clob_select(dbc, file_id, argv[3]);
    if (!SQL_SUCCEEDED(rc))
    {
        goto EXIT_DBC;
    }
...
SQLRETURN execute_clob_insert(SQLHDBC dbc, int src_file_id, const char *src_filename)
{
    ...
    rc = SQLPrepare(stmt, (SQLCHAR *)"INSERT INTO demo_clob VALUES (?, ?)", SQL_NTS);
    if (!SQL_SUCCEEDED(rc))
    {
...
SQLRETURN execute_clob_select(SQLHDBC dbc, int desc_file_id, const char *desc_filename)
{
    ...
    SQLUINTEGER  clob_length;
    SQLUBIGINT   clob_loc = 0;
    ...
    rc = SQLExecDirect(stmt, (SQLCHAR*)"SELECT val FROM demo_clob WHERE id = ?", SQL_NTS);
    if (!SQL_SUCCEEDED(rc))
    {
    ...
    rc = SQLBindCol(stmt, 1, SQL_C_CLOB_LOCATOR, &clob_loc, 0, NULL);
    if (!SQL_SUCCEEDED(rc))
    {
    ...
    rc = SQLGetLobLength(stmt, clob_loc, SQL_C_CLOB_LOCATOR, &clob_length);
    if (!SQL_SUCCEEDED(rc))
    {
    ...
    for (getlob_offset = 0; getlob_offset < clob_length;)
    {
        if (getlob_offset + LOB_BUF_SIZE < clob_length)
        {
            getlob_size = LOB_BUF_SIZE;
        }
        else
        {
            getlob_size = clob_length - getlob_offset;
        }

        rc = SQLGetLob(stmt, SQL_C_CLOB_LOCATOR, clob_loc, getlob_offset, getlob_size, SQL_C_CHAR, getlob_buf, LOB_BUF_SIZE, NULL);
        if (!SQL_SUCCEEDED(rc))
        {
